...
| `frame` | integer | zero-based frame | Shared temporal index. |
...
Its suffix is derived from content including dataset and sequence identity, event count, source provenance, configuration references, validation summary and output hashes. Wall clock time, random values and machine specific paths are excluded.
