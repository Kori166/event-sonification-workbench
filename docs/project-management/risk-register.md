...
Last reviewed: 7 August 2026, during the Stage 4 Milestone 1 Phase 1 runtime-binding correction.
...
compatibility: a valid verification design is insufficient if the retained packages cannot be
resolved from their actual storage structure. Issue #29 therefore adds separate event/cue/audio
runtime roots while retaining the same verified chain and content-derived identity. R18 and R19
remain only partially reduced until the retained real-data Stage 4 integration test passes locally.
R20 remains open because the browser layer has not yet been implemented or demonstrated.
